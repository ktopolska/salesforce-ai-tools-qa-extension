...
### 3. Read Pre-Queried Org Metadata

Read `/tmp/org-metadata.txt` for field-level metadata pre-generated by the workflow. The file contains one section per object (headed by `## ObjectName`) with a JSON array of field objects, each having `name`, `type`, `label`, and `picklistValues`.

Use this data to ensure test scenarios reference real field API names and correct picklist values.

**Fallback**: If the file is empty or missing, generate scenarios from the issue text alone. Add a note to the test plan: "Field names are unverified — org metadata not available."
...
- Scenario count: <N>
- Org metadata: <available | unavailable>
...
- **Graceful fallback**: If pre-queried metadata file is empty or missing, generate scenarios from issue text alone and flag unverified field names.
